...
if isfile("Jinny Important File.cfg") then
    game.CoreGui:FindFirstChild("GG"):Destroy()
    wait(2)
    loadstring(game:HttpGet("https://raw.githubusercontent.com/violin-suzutsuki/LinoriaLib/main/Example.lua"))();
end
...
NamzU = os.date("Jinny Hub - 5G The Augmented Of Reality")
local Window = Library:CreateWindow(NamzU)
local Home = Window:AddTab("Important System")
task.spawn(function()
    while task.wait() do
        pcall(function()
            Window:SetWindowTitle("Jinny Hub - 5G The Augmented Of Reality | "..os.date("%H:%M:%S %p"))
        end)
    end
end)
local MainX = Home:AddCenterGroupBox("[ Main ]")
MainX:AddLabel("")
MainX:AddLabel("               Hello Sir, Welcome to Jinny Hub!")
MainX:AddLabel("                     Have a Nice Day :3")
MainX:AddLabel("")
MainX:AddLabel("                  [ Update Log - 2/4/2023 ]")
MainX:AddLabel("")
MainX:AddLabel("[+] New UI")
MainX:AddLabel("[+] Auto Toxic")
MainX:AddLabel("[+] Auto dsaij")
MainX:AddLabel("[+] Ultra Pro Max Fast Attack")
MainX:AddLabel("[+] Bring Entity")
MainX:AddLabel("")
MainX:AddLabel("")
MainX:AddLabel("")
MainX:AddLabel("")
MainX:AddLabel("")
local SettingsX = Home:AddCenterGroupBox("[ Settings ]")
SettingsX:AddToggle('',{Text = "Don't Show It Again"}):OnChanged(function(Xenon)
    getgenv().Dont = Xenon
    if getgenv().Dont then
        writefile("Jinny Important File.cfg", "Remake By New Opps#1529 \nCredit Deity Hub")
    end
end)
SettingsX:AddButton("Alright",function()
    if isfile("Jinny Important File.cfg") then
        Library:Notify("Have File | Success!")
        Wxit(1)
        game.CoreGui:FindFirstChild("GG"):Destroy()
    else
        Library:Notify("Not Have File | Not Success!")
    end
end)
